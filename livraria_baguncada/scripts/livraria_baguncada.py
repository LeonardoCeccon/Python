...
# Exibe o diretório atual (útil para saber onde o Python está rodando)
print(os.getcwd())
...
# Exibe as 5 primeiras linhas para inspeção inicial
print(df.head())
...
# Remove espaços nas bordas, transforma em minúsculas e troca espaços por underline
df.columns = df.columns.str.strip().str.lower().str.replace(' ', '_')

# Corrige e padroniza os dados de texto nas colunas 'autor' e 'categoria'
# (ATENÇÃO: A linha abaixo substitui 'autor' por 'categoria', o que pode ser um erro!)
df['autor'] = df['categoria'].astype(str).str.strip().str.title()
df['categoria'] = df['categoria'].str.strip().str.title()

# Substitui valores nulos representados como strings por valores nulos reais (pd.NA)
